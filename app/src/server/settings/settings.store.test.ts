...
test('settings default to following the host and persist theme and locale', () => {
  const directory = mkdtempSync(join(tmpdir(), 'ramify-settings-'));
  try {
    const store = new SettingsStore(directory);
    assert.deepEqual(store.read(), { theme: 'system', locale: 'system' });
...
test('invalid settings fall back without rewriting user data', () => {
  ...
  try {
    ...
    assert.deepEqual(new SettingsStore(directory).read(), { theme: 'system', locale: 'system' });
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
